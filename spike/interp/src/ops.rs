...
fn bin(env: &Env, n: &Node, f: unsafe extern "C" fn(*mut mlx_array, mlx_array, mlx_array, mlx_stream) -> i32) -> mlx_array {
    let a = ga(env, &n.input[0]);
    let b = ga(env, &n.input[1]);
    op!(f, a, b)
}
fn un(env: &Env, n: &Node, f: unsafe extern "C" fn(*mut mlx_array, mlx_array, mlx_stream) -> i32) -> mlx_array {
    let a = ga(env, &n.input[0]);
    op!(f, a)
}

fn run_node(n: &Node, env: &mut Env, refmap: &HashMap<String, mlx_array>) -> Vec<(String, Val)> {
    ...
    match n.op.as_str() {
        ...
        "Expand" => {
            ...
            // bidirectional broadcast of cur and tgt
        ...
        "ConstantOfShape" => {
            ...
            let (val, dt) = if let Some(key) = n.at("value") {
                let t = env_arr(env, refmap, &key);
                (t, dtype(t))
            } else {
                (scalar_f32(0.0), F32)
            };
        ...
        "Loop" => loop_op(env, n, refmap),
        "If" => {
            let cond = read_i64(ga(env, &n.input[0]))[0] != 0;
            let key = if cond { "then_branch" } else { "else_branch" };
            let br: Subgraph = serde_json::from_value(n.attr[key].clone()).unwrap();
            let mut ce = env.clone();
            exec_nodes(&br.nodes, &mut ce, refmap);
            n.output
                .iter()
                .zip(br.output.iter())
        ...
        "TopK" => {
            ...
            for row in 0..rows {
                ...
                order.sort_by(|&i, &j| {
                    seg[j].partial_cmp(&seg[i]).unwrap().then(i.cmp(&j))
                });
                if !largest {
        ...
        // RNG nodes are unseeded in ONNX (nondeterministic). For parity we inject
        // the reference run's captured output so the rest of the graph is exact.
        // (Production would use mlx_random here — it's a noise source by design.)
        "Pad" => {
            let a = ga(env, &n.input[0]);
            ...
            let mode = n.at("mode").unwrap_or_else(|| "constant".into());
            if mode == "reflect" {
        ...
        "Slice" => {
            ...
            let dims = shape(a);
            // per-axis (start, stop, step) under ONNX clamping
            let mut sss: Vec<(i64, i64, i64)> =
                (0..r).map(|i| (0, dims[i] as i64, 1)).collect();
            for k in 0..axes.len() {
                let mut ax = axes[k];
                if ax < 0 {
                    ax += r as i64;
        ...
        other => {
            println!("UNIMPLEMENTED op '{other}' (node '{}')", n.name);
            std::process::exit(2);
        }
    }
}

fn env_arr(env: &Env, refmap: &HashMap<String, mlx_array>, key: &str) -> mlx_array {
    if let Some(Val::A(a)) = env.get(key) {
        *a
    } else if let Some(a) = refmap.get(key) {
        *a
    } else {
        panic!("missing const tensor {key}")
    }
}
...
fn conv(env: &Env, n: &Node) -> mlx_array {
    ...
    let stride = *strides.get(0).unwrap_or(&1) as i32;
    let dil = *dils.get(0).unwrap_or(&1) as i32;
...
fn resize(env: &Env, n: &Node) -> mlx_array {
    let a = ga(env, &n.input[0]);
    let mode = n.at("mode").unwrap_or_else(|| "nearest".into());
    let dims = shape(a);
    ...
    if mode == "nearest" {
        ...
        // linear interpolation along the last axis (half_pixel)
        let r = dims.len();
        let last = dims[r - 1] as usize;
...
fn conv_transpose(env: &Env, n: &Node) -> mlx_array {
    ...
    let groups = n.ai("group", 1) as i32;
    let stride = *n.aints("strides").get(0).unwrap_or(&1) as i32;
    let dil = *n.aints("dilations").get(0).unwrap_or(&1) as i32;
    let outpad = *n.aints("output_padding").get(0).unwrap_or(&0) as i32;
    let pads = n.aints("pads");
    let pad = if pads.is_empty() { 0 } else { pads[0] as i32 };
    let xnlc = transpose(x, &[0, 2, 1]); // [N, L, Cin]
    // ONNX ConvTranspose weight [Cin, Cout/g, K] -> MLX [Cout, K, Cin/g].
    // General grouped layout: [g, Cin/g, Cout/g, K] -> [g, Cout/g, K, Cin/g].
    let ws = shape(w);
...
// ONNX LSTM (forward or bidirectional), gate order iofc. Validated in spike/parity.c.
fn lstm(env: &Env, n: &Node) -> Vec<(String, Val)> {
    ...
    let dir = n.at("direction").unwrap_or("forward".into());
    let ndir = if dir == "bidirectional" { 2 } else { 1 };
    let g4 = 4 * hidden;

    let xd = read_f32(x); // seq*batch*in
    let wd = read_f32(w);
    let rd = read_f32(r);
    ...
    for d in 0..ndir {
        ...
        let wb = bd.as_ref().map(|v| (d * 8 * hidden) as usize);
        for bt in 0..batch {
            let mut h = vec![0f32; hidden as usize];
            let mut c = vec![0f32; hidden as usize];
            for k in 0..seq {
                let t = if d == 0 { k } else { seq - 1 - k };
                // gates[g] = sum_in x[t,bt,in]*W[d,g,in] + sum_h h*R[d,g,h] + Wb+Rb
                ...
                for j in 0..hs {
                    let i = sigmoidf(gate[j]); // input
                    let o = sigmoidf(gate[hs + j]); // output
                    let f = sigmoidf(gate[2 * hs + j]); // forget
                    let cc = gate[3 * hs + j].tanh(); // cell
                    c[j] = f * c[j] + i * cc;
                    h[j] = o * c[j].tanh();
                }
                for j in 0..hs {
...
fn exec_nodes(nodes: &[Node], env: &mut Env, refmap: &HashMap<String, mlx_array>) {
    for n in nodes {
        let outs = run_node(n, env, refmap);
        for (k, v) in outs {
            env.insert(k, v);
        }
    }
...
// This graph's loop has no scan outputs; outputs are the final carried values.
fn loop_op(env: &Env, n: &Node, refmap: &HashMap<String, mlx_array>) -> Vec<(String, Val)> {
    let m = read_i64(ga(env, &n.input[0]))[0];
    let mut cond = read_i64(ga(env, &n.input[1]))[0] != 0;
    let body: Subgraph = serde_json::from_value(n.attr["body"].clone()).unwrap();
    let ncarry = n.input.len() - 2;
    let mut carried: Vec<Val> = n.input[2..]
        .iter()
        .map(|i| env.get(i).cloned().unwrap())
        .collect();
    let mut iter = 0i64;
    while iter < m && cond {
        let mut ce = env.clone();
        ...
        exec_nodes(&body.nodes, &mut ce, refmap);
...
// ONNX Gather = np.take(data, indices, axis). Host implementation (the mlx-c
// take/take_axis semantics are ambiguous for multi-dim data).
// result shape = data[:axis] + indices.shape + data[axis+1:].
fn gather_host(env: &Env, n: &Node) -> mlx_array {
    ...
    let qv: Vec<i64> = read_i64(idx)
        .iter()
        .map(|&v| if v < 0 { v + dim } else { v })
        .collect();
...
// Host N-D slice supporting negative steps (mlx's stop sentinel for reverse
// doesn't match ONNX's "past the beginning" end value).
...
// ONNX reflect pad (no edge repeat); mlx_pad doesn't support reflect. Host gather.
...
// ONNX ScatterND (reduction=none): out = data with updates written at the
// (k = indices.shape[-1])-dim index vectors. Host implementation for exactness.
...
// ONNX ScatterElements (reduction=none): out[idx-along-axis] = updates, where
// data/indices/updates share rank. Host implementation.
fn scatter_elements(env: &Env, n: &Node) -> mlx_array {
    ...
    let dim = ds[axis] as i64;
    let is_int = dtype(data) == INT64 || dtype(data) == INT32;
    // decompose linear index over indices-shape into multi-index, map to data
